...
import ir.mostafa.launcher.plugin.PluginState
...
internal class PluginServiceImpl(
    ...
    override suspend fun getPluginPackageIcon(plugin: PluginPackage): Drawable? {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val iconRes = appInfo.metaData?.getInt("ir.mostafa.launcher.plugin.icon") ?: 0
                if (iconRes != 0) {
        ...
        return repository.findMany().map {
            ...
            packageGroups.mapNotNull { (packageName, plugins) ->
                ...
                val settingsActivity = context.packageManager.queryIntentActivities(
                    Intent().apply {
                        `package` = packageName
                        action = "ir.mostafa.launcher.action.PLUGIN_SETTINGS"
                    },
                    0
                ).firstOrNull()
                val signature = getSignature(packageName)
                val author = appInfo.metaData?.getString("ir.mostafa.launcher.plugin.author")
                PluginPackage(
                    packageName = packageName,
                    label = appInfo.metaData?.getString("ir.mostafa.launcher.plugin.label")
                        ?: appInfo.loadLabel(context.packageManager).toString(),
                    description = appInfo.metaData?.getString("ir.mostafa.launcher.plugin.description"),
                    author = author,
                    plugins = plugins,
    ...
    override fun getPluginPackage(packageName: String): Flow<PluginPackage?> {
        ...
        val settingsActivityInfo = context.packageManager.queryIntentActivities(
            Intent().apply {
                `package` = packageName
                action = "dir.mostafa.launcher.action.PLUGIN_SETTINGS"
            },
            0
        ).firstOrNull()
        val signature = getSignature(packageName)
        val author = appInfo.metaData?.getString("dir.mostafa.launcher.plugin.author")
        return repository.findMany(packageName = packageName)
            .map {
                PluginPackage(
                    packageName = packageName,
                    label = appInfo.metaData?.getString("dir.mostafa.launcher.plugin.label")
                        ?: appInfo.loadLabel(context.packageManager).toString(),
                    description = appInfo.metaData?.getString("dir.mostafa.launcher.plugin.description"),
                    author = author,
                    plugins = it,
